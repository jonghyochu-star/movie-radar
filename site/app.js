...
function videosForTab(){
 const q=$('#search').value.trim().toLowerCase(), min=Number($('#min-views').value);
 let items=allVideos().filter(v=>dataset.mode==='demo'||!sample(v));
 const collected=new Set(dataset.videos.map(v=>v.id));
 items=items.filter(v=>{
   const r=historyRecord(v), origin=C.originOf(r);
   if(tab==='history')return Boolean(r.rating||r.stage||r.memo||r.label||origin!=='unknown');
   if(tab==='done')return r.stage==='done';
   if(origin==='korean')return false;
   if(tab==='review')return C.needsReview(v,r)&&!skipped.has(v.id)&&(min===0||(v.views??-1)>=min);
   if(tab==='discover')return collected.has(v.id)&&C.ready(v,r)&&!r.rating&&!r.stage&&!skipped.has(v.id)&&(min===0||(v.views??-1)>=min);
   if(tab==='likes')return r.rating==='like';
   if(tab==='candidate')return r.stage==='candidate'&&C.ready(v,r);
   return false;
 });
...
function render(){
 ...
 $('#content').innerHTML=slice.length?slice.map(renderCard).join(''):'<div class="empty-state" style="grid-column:1/-1"><h3>이 조건의 소재가 없습니다.</h3><p>처음 수집한 영상은 원작 확인 필요 탭에 있습니다.<br>검색·조회수 조건을 풀거나 다른 탭을 확인하세요.<br>새 수집은 GitHub Actions에서 실행합니다. 목록 새로고침은 배포된 목록만 다시 읽습니다.</p></div>';
...
function handleAction(id,action){const v=allVideos().find(x=>x.id===id);if(!v)return;if(action==='memo'){editedId=id;$('#edit-label').value=historyRecord(v).label||'';$('#edit-memo').value=historyRecord(v).memo||'';$('#edit-dialog').showModal();return;}snapshot();if(action==='later'){skipped.add(id);render();toast('이번 탐색에서만 넘겼습니다. 취향 평가에는 쓰지 않습니다.',true);return;}if(['origin_foreign','origin_korean'].includes(action)&&!confirm(action==='origin_foreign'?'원작을 확인했고, 한국 영화가 아닌 작품이 맞나요? 영상 언어·채널 국가만으로 판단하지 마세요.':'원작이 한국 영화임을 확인했나요? 이 영상만 추천·보관함·제작 후보에서 숨기며, 메모와 기존 기록은 보존합니다.'))return;try{state=C.apply(state,v,action);}catch(error){toast(error.message);return;}const ok=persist();render();if(ok)toast({like:'좋아요 보관함에 저장했습니다.',dislike:'별로로 기록했습니다.',candidate:'제작 후보에 넣었습니다.',uncandidate:'후보만 해제했습니다. 좋아요는 유지됩니다.',done:'제작 완료로 기록했습니다.',reopen:'제작 후보로 되돌렸습니다.',unrate:'취향 평가를 취소했습니다.',origin_foreign:'한국 외 원작으로 기록했습니다. 소재 탐색에서 검토할 수 있습니다.',origin_korean:'한국 영화로 제외했습니다. 평가 기록에서 되돌릴 수 있습니다.',origin_reset:'원작 확인을 취소했습니다. 확인 필요 탭으로 분리합니다.'}[action],true);}
